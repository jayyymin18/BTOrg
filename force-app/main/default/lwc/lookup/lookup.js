...
import searchLookupRecords from "@salesforce/apex/LookupController.searchLookupRecords";
import getRecentlyCreatedRecord from "@salesforce/apex/LookupController.getRecentlyCreatedRecord";
...
export default class Lookup extends LightningElement {
  ...
  handleSuccess(event) {
    this.notifyUser(
      "Success",
      "Successfully created " + this.labelOfObject,
      "success"
    );
    ...
    getRecentlyCreatedRecord(JSON.parse(JSON.stringify(params)))
      .then((result) => {
        this.isNewRecordForm = false;
    ...
    this.searchThrottlingTimeout = setTimeout(
      (self) => {
        ...
        if (self.cleanSearchTerm.length >= MINIMAL_SEARCH_TERM_LENGTH) {
          self.handleSearch(
            self.cleanSearchTerm,
            self.selection.map((element) => element.id)
          );
        }
  ...
  handleSearch(searchTerm, selectedIds) {
    ...
    searchLookupRecords(JSON.parse(JSON.stringify(parameters)))
      .then((results) => {
  ...
  handleFocus() {
    ...
    [...this.template.querySelectorAll(".dropdown")].map((li) =>
      li.firstChild.classList.remove(HAS_FOCUS)
    );
    this.focusIndex = -1;
    this.hasFocus = true;
  }
  ...
  arrowNavigationUp(event) {
    ...
    if (
      this.hasFocus &&
      (this.hasResults() || this.noRecordFound) &&
  ...
  get getInputClass() {
    ...
    if (!this.isMultiSelect) {
      css += this.hasSelection()
        ? "slds-combobox__input-value has-custom-border"
        : "";
    }
    return css;
  }
  ...
  get pillsContainer() {
    return this.isMultiSelect && this.hasSelection()
      ? "slds-listbox_selection-group selectionGroup"
      : "";
  }
